// ------------------------------------------------------------------------------------------------------
// <copyright file="AcalaExplorerService.cs" company="Nomis">
...
using Nomis.AcalaExplorer.Calculators;
using Nomis.AcalaExplorer.Interfaces;
using Nomis.AcalaExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IAcalaScoringService"/>
    internal sealed class AcalaExplorerService :
        BlockchainDescriptor,
        IAcalaScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IAcalaExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="AcalaExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="AcalaExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IAcalaExplorerClient"/>.</param>
        ...
        public AcalaExplorerService(
            IOptions<AcalaExplorerSettings> settings,
            IAcalaExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "acala";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "acala";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToAca() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<AcalaExplorerAccountNormalTransactions, AcalaExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<AcalaExplorerAccountERC20TokenEvents, AcalaExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new AcalaStatCalculator(
                    request.Address,
