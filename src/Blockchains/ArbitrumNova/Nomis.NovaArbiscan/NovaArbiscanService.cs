// ------------------------------------------------------------------------------------------------------
// <copyright file="NovaArbiscanService.cs" company="Nomis">
...
using Nomis.NovaArbiscan.Calculators;
...
{
    /// <inheritdoc cref="IArbitrumNovaScoringService"/>
    internal sealed class NovaArbiscanService :
        BlockchainDescriptor,
        IArbitrumNovaScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly INovaArbiscanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="NovaArbiscanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="NovaArbiscanSettings"/>.</param>
        /// <param name="client"><see cref="INovaArbiscanClient"/>.</param>
        ...
        public NovaArbiscanService(
            IOptions<NovaArbiscanSettings> settings,
            INovaArbiscanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "arbitrum_nova";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            var transactions = (await _client.GetTransactionsAsync<NovaArbiscanAccountNormalTransactions, NovaArbiscanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<NovaArbiscanAccountInternalTransactions, NovaArbiscanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<NovaArbiscanAccountERC20TokenEvents, NovaArbiscanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<NovaArbiscanAccountERC721TokenEvents, NovaArbiscanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new ArbitrumNovaStatCalculator(
                    request.Address,
