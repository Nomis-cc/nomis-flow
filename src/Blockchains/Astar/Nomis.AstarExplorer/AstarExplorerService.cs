// ------------------------------------------------------------------------------------------------------
// <copyright file="AstarExplorerService.cs" company="Nomis">
...
using Nomis.AstarExplorer.Calculators;
using Nomis.AstarExplorer.Interfaces;
using Nomis.AstarExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IAstarScoringService"/>
    internal sealed class AstarExplorerService :
        BlockchainDescriptor,
        IAstarScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IAstarExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="AstarExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="AstarExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IAstarExplorerClient"/>.</param>
        ...
        public AstarExplorerService(
            IOptions<AstarExplorerSettings> settings,
            IAstarExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "astar";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "astar";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToAstr() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<AstarExplorerAccountNormalTransactions, AstarExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<AstarExplorerAccountERC20TokenEvents, AstarExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new AstarStatCalculator(
                    request.Address,
