// ------------------------------------------------------------------------------------------------------
// <copyright file="AurorascanService.cs" company="Nomis">
...
using Nomis.Aurorascan.Calculators;
using Nomis.Aurorascan.Interfaces;
using Nomis.Aurorascan.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IAuroraScoringService"/>
    internal sealed class AurorascanService :
        BlockchainDescriptor,
        IAuroraScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IAurorascanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="AurorascanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="AurorascanSettings"/>.</param>
        /// <param name="client"><see cref="IAurorascanClient"/>.</param>
        ...
        public AurorascanService(
            IOptions<AurorascanSettings> settings,
            IAurorascanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "aurora";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            var transactions = (await _client.GetTransactionsAsync<AurorascanAccountNormalTransactions, AurorascanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<AurorascanAccountInternalTransactions, AurorascanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<AurorascanAccountERC20TokenEvents, AurorascanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = new List<IAurorascanAccountNftTokenEvent>(); // (await _client.GetTransactionsAsync<AurorascanAccountERC721TokenEvents, AurorascanAccountERC721TokenEvent>(request.Address)).ToList();
            ...
            var walletStats = new AuroraStatCalculator(
                    request.Address,
