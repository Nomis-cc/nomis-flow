...
namespace Nomis.Snowtrace
{
        ...
        ITransientService
    {
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "avax";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "avalanche-2";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToAvax() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<SnowtraceAccountNormalTransactions, SnowtraceAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<SnowtraceAccountInternalTransactions, SnowtraceAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<SnowtraceAccountERC20TokenEvents, SnowtraceAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<SnowtraceAccountERC721TokenEvents, SnowtraceAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new AvalancheStatCalculator(
                    request.Address,
