// ------------------------------------------------------------------------------------------------------
// <copyright file="BttcscanService.cs" company="Nomis">
...
using Nomis.Bttcscan.Calculators;
...
{
    /// <inheritdoc cref="IBitTorrentScoringService"/>
    internal sealed class BttcscanService :
        BlockchainDescriptor,
        IBitTorrentScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IBttcscanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="BttcscanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="BttcscanSettings"/>.</param>
        /// <param name="client"><see cref="IBttcscanClient"/>.</param>
        ...
        public BttcscanService(
            IOptions<BttcscanSettings> settings,
            IBttcscanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "bittorrent";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "bittorrent";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToBtt() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<BttcscanAccountNormalTransactions, BttcscanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<BttcscanAccountInternalTransactions, BttcscanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<BttcscanAccountERC20TokenEvents, BttcscanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<BttcscanAccountERC721TokenEvents, BttcscanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new BitTorrentStatCalculator(
                    request.Address,
