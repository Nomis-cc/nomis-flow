// ------------------------------------------------------------------------------------------------------
// <copyright file="BitTorrentStatCalculator.cs" company="Nomis">
...
namespace Nomis.Bttcscan.Calculators
{
    /// <summary>
    /// BitTorrent wallet stats calculator.
    /// </summary>
    internal sealed class BitTorrentStatCalculator :
        IStatCalculator<BitTorrentWalletStats, BitTorrentTransactionIntervalData>
    {
        private readonly string _address;
        private readonly decimal _balance;
        private readonly decimal _usdBalance;
        private readonly IEnumerable<BttcscanAccountNormalTransaction> _transactions;
        private readonly IEnumerable<BttcscanAccountInternalTransaction> _internalTransactions;
        private readonly IEnumerable<IBttcscanAccountNftTokenEvent> _tokenTransfers;
        private readonly IEnumerable<BttcscanAccountERC20TokenEvent> _erc20TokenTransfers;
        ...
        public BitTorrentStatCalculator(
            string address,
            decimal balance,
            decimal usdBalance,
            IEnumerable<BttcscanAccountNormalTransaction> transactions,
            IEnumerable<BttcscanAccountInternalTransaction> internalTransactions,
            IEnumerable<IBttcscanAccountNftTokenEvent> tokenTransfers,
            IEnumerable<BttcscanAccountERC20TokenEvent> erc20TokenTransfers,
        ...
        public BitTorrentWalletStats GetStats()
        {
            ...
            var turnoverIntervals = IStatCalculator<BitTorrentTransactionIntervalData>
                .GetTurnoverIntervals(turnoverIntervalsDataList, _transactions.Min(x => x.TimeStamp!.ToDateTime())).ToList();

            return new()
            {
                NativeBalance = _balance.ToBtt(),
                ...
                WalletTurnover = _transactions.Sum(x => decimal.TryParse(x.Value, out decimal value) ? value.ToBtt() : 0),
                BalanceChangeInLastMonth = IStatCalculator<BitTorrentTransactionIntervalData>.GetBalanceChangeInLastMonth(turnoverIntervals),
                BalanceChangeInLastYear = IStatCalculator<BitTorrentTransactionIntervalData>.GetBalanceChangeInLastYear(turnoverIntervals),
                ...
                NftTrading = (soldSum - buySum).ToBtt(),
                NftWorth = nftWorth.ToBtt(),
