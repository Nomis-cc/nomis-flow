...
namespace Nomis.Bobascan
{
        ...
        ITransientService
    {
        ...
        /// </summary>
        /// <param name="settings"><see cref="BobascanSettings"/>.</param>
        ...
        public BobascanService(
            IOptions<BobascanSettings> settings,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "boba";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            var transactions = (await _client.GetTransactionsAsync<BobascanAccountNormalTransactions, BobascanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<BobascanAccountInternalTransactions, BobascanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<BobascanAccountERC20TokenEvents, BobascanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokenTransfers = new List<IBobascanAccountNftTokenEvent>();
            var erc721Tokens = (await _client.GetTransactionsAsync<BobascanAccountERC721TokenEvents, BobascanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var erc1155Tokens = (await _client.GetTransactionsAsync<BobascanAccountERC1155TokenEvents, BobascanAccountERC1155TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
