// ------------------------------------------------------------------------------------------------------
// <copyright file="CantoStatCalculator.cs" company="Nomis">
...
namespace Nomis.CantoExplorer.Calculators
{
    /// <summary>
    /// Canto wallet stats calculator.
    /// </summary>
    internal sealed class CantoStatCalculator :
        IStatCalculator<CantoWalletStats, CantoTransactionIntervalData>
    {
        private readonly string _address;
        private readonly decimal _balance;
        private readonly decimal _usdBalance;
        private readonly IEnumerable<CantoExplorerAccountNormalTransaction> _transactions;
        private readonly IEnumerable<CantoExplorerAccountERC20TokenEvent> _erc20TokenTransfers;
        ...
        public CantoStatCalculator(
            string address,
            decimal balance,
            decimal usdBalance,
            IEnumerable<CantoExplorerAccountNormalTransaction> transactions,
            IEnumerable<CantoExplorerAccountERC20TokenEvent> erc20TokenTransfers,
        ...
        public CantoWalletStats GetStats()
        {
            ...
            var turnoverIntervals = IStatCalculator<CantoTransactionIntervalData>
                .GetTurnoverIntervals(turnoverIntervalsDataList, _transactions.Min(x => x.TimeStamp!.ToDateTime())).ToList();

            return new()
            {
                NativeBalance = _balance.ToCanto(),
                ...
                WalletTurnover = _transactions.Sum(x => decimal.TryParse(x.Value, out decimal value) ? value.ToCanto() : 0),
                BalanceChangeInLastMonth = IStatCalculator<CantoTransactionIntervalData>.GetBalanceChangeInLastMonth(turnoverIntervals),
                BalanceChangeInLastYear = IStatCalculator<CantoTransactionIntervalData>.GetBalanceChangeInLastYear(turnoverIntervals),
