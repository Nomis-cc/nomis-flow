// ------------------------------------------------------------------------------------------------------
// <copyright file="CantoExplorerService.cs" company="Nomis">
...
using Nomis.CantoExplorer.Calculators;
using Nomis.CantoExplorer.Interfaces;
using Nomis.CantoExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="ICantoScoringService"/>
    internal sealed class CantoExplorerService :
        BlockchainDescriptor,
        ICantoScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly ICantoExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="CantoExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="CantoExplorerSettings"/>.</param>
        /// <param name="client"><see cref="ICantoExplorerClient"/>.</param>
        ...
        public CantoExplorerService(
            IOptions<CantoExplorerSettings> settings,
            ICantoExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "canto";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "canto";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToCanto() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<CantoExplorerAccountNormalTransactions, CantoExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<CantoExplorerAccountERC20TokenEvents, CantoExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new CantoStatCalculator(
                    request.Address,
