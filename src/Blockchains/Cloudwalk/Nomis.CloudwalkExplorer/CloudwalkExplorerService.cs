// ------------------------------------------------------------------------------------------------------
// <copyright file="CloudwalkExplorerService.cs" company="Nomis">
...
using Nomis.CloudwalkExplorer.Calculators;
...
{
    /// <inheritdoc cref="ICloudwalkScoringService"/>
    internal sealed class CloudwalkExplorerService :
        BlockchainDescriptor,
        ICloudwalkScoringService,
        ITransientService
    {
        private readonly ICloudwalkExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="CloudwalkExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="CloudwalkExplorerSettings"/>.</param>
        /// <param name="client"><see cref="ICloudwalkExplorerClient"/>.</param>
        ...
        public CloudwalkExplorerService(
            IOptions<CloudwalkExplorerSettings> settings,
            ICloudwalkExplorerClient client,
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            // (await _defiLlamaService.GetTokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToCwn() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<CloudwalkExplorerAccountNormalTransactions, CloudwalkExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<CloudwalkExplorerAccountERC20TokenEvents, CloudwalkExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new CloudwalkStatCalculator(
                    request.Address,
