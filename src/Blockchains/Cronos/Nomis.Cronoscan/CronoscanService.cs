// ------------------------------------------------------------------------------------------------------
// <copyright file="CronoscanService.cs" company="Nomis">
...
using Nomis.Cronoscan.Calculators;
using Nomis.Cronoscan.Interfaces;
using Nomis.Cronoscan.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="ICronosScoringService"/>
    internal sealed class CronoscanService :
        BlockchainDescriptor,
        ICronosScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly ICronoscanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="CronoscanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="CronoscanSettings"/>.</param>
        /// <param name="client"><see cref="ICronoscanClient"/>.</param>
        ...
        public CronoscanService(
            IOptions<CronoscanSettings> settings,
            ICronoscanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "cronos";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "crypto-com-chain";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToCro() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<CronoscanAccountNormalTransactions, CronoscanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<CronoscanAccountInternalTransactions, CronoscanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var crc20Tokens = (await _client.GetTransactionsAsync<CronoscanAccountCRC20TokenEvents, CronoscanAccountCRC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<CronoscanAccountCRC721TokenEvents, CronoscanAccountCRC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            {
                foreach (string? crc20TokenContractId in crc20Tokens.Select(x => x.ContractAddress).Distinct())
                {
                    await Task.Delay(250, cancellationToken).ConfigureAwait(false);
                    var tokenBalance = (await _client.GetTokenBalanceAsync(request.Address, crc20TokenContractId!).ConfigureAwait(false)).Balance?.ToBigInteger();
                    if (tokenBalance > 0)
                    {
                        tokensData.Add((crc20TokenContractId!, $"{DefiLLamaChainId}:{crc20TokenContractId}", tokenBalance));
                    }
                }
            }
            ...
            var walletStats = new CronosStatCalculator(
                    request.Address,
