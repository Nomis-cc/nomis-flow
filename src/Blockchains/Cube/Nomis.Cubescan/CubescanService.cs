...
namespace Nomis.Cubescan
{
        ...
        ITransientService
    {
        ...
        /// </summary>
        /// <param name="settings"><see cref="CubescanSettings"/>.</param>
        ...
        public CubescanService(
            IOptions<CubescanSettings> settings,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "cube";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "cube-network";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToCube() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<CubescanAccountNormalTransactions, CubescanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<CubescanAccountInternalTransactions, CubescanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();

            // TODO - fix later
            var erc20Tokens = new List<CubescanAccountERC20TokenEvent>(); // (await _client.GetTransactionsAsync<CubescanAccountERC20TokenEvents, CubescanAccountERC20TokenEvent>(request.Address)).ToList(); // TODO - 504 http status from API
            var tokens = (await _client.GetTransactionsAsync<CubescanAccountERC721TokenEvents, CubescanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
