// ------------------------------------------------------------------------------------------------------
// <copyright file="DogechainStatCalculator.cs" company="Nomis">
...
namespace Nomis.DogechainExplorer.Calculators
{
    /// <summary>
    /// Dogechain wallet stats calculator.
    /// </summary>
    internal sealed class DogechainStatCalculator :
        IStatCalculator<DogechainWalletStats, DogechainTransactionIntervalData>
    {
        private readonly string _address;
        private readonly decimal _balance;
        private readonly decimal _usdBalance;
        private readonly IEnumerable<DogechainExplorerAccountNormalTransaction> _transactions;
        private readonly IEnumerable<DogechainExplorerAccountERC20TokenEvent> _erc20TokenTransfers;
        ...
        public DogechainStatCalculator(
            string address,
            decimal balance,
            decimal usdBalance,
            IEnumerable<DogechainExplorerAccountNormalTransaction> transactions,
            IEnumerable<DogechainExplorerAccountERC20TokenEvent> erc20TokenTransfers,
        ...
        public DogechainWalletStats GetStats()
        {
            ...
            var turnoverIntervals = IStatCalculator<DogechainTransactionIntervalData>
                .GetTurnoverIntervals(turnoverIntervalsDataList, _transactions.Min(x => x.TimeStamp!.ToDateTime())).ToList();

            return new()
            {
                NativeBalance = _balance.ToDoge(),
                ...
                WalletTurnover = _transactions.Sum(x => decimal.TryParse(x.Value, out decimal value) ? value.ToDoge() : 0),
                BalanceChangeInLastMonth = IStatCalculator<DogechainTransactionIntervalData>.GetBalanceChangeInLastMonth(turnoverIntervals),
                BalanceChangeInLastYear = IStatCalculator<DogechainTransactionIntervalData>.GetBalanceChangeInLastYear(turnoverIntervals),
