// ------------------------------------------------------------------------------------------------------
// <copyright file="DogechainExplorerService.cs" company="Nomis">
...
using Nomis.DogechainExplorer.Calculators;
...
{
    /// <inheritdoc cref="IDogechainScoringService"/>
    internal sealed class DogechainExplorerService :
        BlockchainDescriptor,
        IDogechainScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IDogechainExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="DogechainExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="DogechainExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IDogechainExplorerClient"/>.</param>
        ...
        public DogechainExplorerService(
            IOptions<DogechainExplorerSettings> settings,
            IDogechainExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "dogechain";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "dogecoin";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToDoge() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<DogechainExplorerAccountNormalTransactions, DogechainExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<DogechainExplorerAccountERC20TokenEvents, DogechainExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new DogechainStatCalculator(
                    request.Address,
