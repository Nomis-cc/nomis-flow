// ------------------------------------------------------------------------------------------------------
// <copyright file="EtcExplorerService.cs" company="Nomis">
...
using Nomis.EtcExplorer.Calculators;
using Nomis.EtcExplorer.Interfaces;
using Nomis.EtcExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IEtcScoringService"/>
    internal sealed class EtcExplorerService :
        BlockchainDescriptor,
        IEtcScoringService,
        ITransientService
    {
        private readonly IEtcExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="EtcExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="EtcExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IEtcExplorerClient"/>.</param>
        ...
        public EtcExplorerService(
            IOptions<EtcExplorerSettings> settings,
            IEtcExplorerClient client,
        ...
        /// </summary>
        public string CoingeckoNativeTokenId => "ethereum-classic";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToEtc() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<EtcExplorerAccountNormalTransactions, EtcExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<EtcExplorerAccountERC20TokenEvents, EtcExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            {
                Blockchain = Chain.EthereumClassic,
            ...
            var walletStats = new EtcStatCalculator(
                    request.Address,
