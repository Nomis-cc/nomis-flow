...
namespace Nomis.EvmosAPI
{
        ...
        ITransientService
    {
        ...
        /// </summary>
        /// <param name="settings"><see cref="EvmosEVMSettings"/>.</param>
        ...
        public EvmosAPIService(
            IOptions<EvmosEVMSettings> settings,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "evmos";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "evmos";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToEvmos() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<EvmosAPIAccountNormalTransactions, EvmosAPIAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<EvmosAPIAccountERC20TokenEvents, EvmosAPIAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokenList = (await _client.GetOwnedTokensAsync(request.Address).ConfigureAwait(false)).ToList();
            ...
            if ((request as IWalletTokensBalancesRequest)?.GetHoldTokensBalances == true)
            {
                var tokens = tokenList
                    ...
                    .DistinctBy(x => x.ContractAddress);
                foreach (var token in tokens)
                {
                    await Task.Delay(250, cancellationToken).ConfigureAwait(false);
                    var tokenBalance = token.Balance?.ToBigInteger();
                    if (tokenBalance > 0)
                    {
                        tokensData.Add((token.ContractAddress!, $"{DefiLLamaChainId}:{token.ContractAddress}", tokenBalance));
                    }
                }
            }
