// ------------------------------------------------------------------------------------------------------
// <copyright file="FuseExplorerService.cs" company="Nomis">
...
using Nomis.FuseExplorer.Calculators;
using Nomis.FuseExplorer.Interfaces;
using Nomis.FuseExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IFuseScoringService"/>
    internal sealed class FuseExplorerService :
        BlockchainDescriptor,
        IFuseScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IFuseExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="FuseExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="FuseExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IFuseExplorerClient"/>.</param>
        ...
        public FuseExplorerService(
            IOptions<FuseExplorerSettings> settings,
            IFuseExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "fuse";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "fuse-network-token";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToFuse() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<FuseExplorerAccountNormalTransactions, FuseExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<FuseExplorerAccountERC20TokenEvents, FuseExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new FuseStatCalculator(
                    request.Address,
