// ------------------------------------------------------------------------------------------------------
// <copyright file="HaqqExplorerService.cs" company="Nomis">
...
using Nomis.HaqqExplorer.Calculators;
...
{
    /// <inheritdoc cref="IHaqqScoringService"/>
    internal sealed class HaqqExplorerService :
        BlockchainDescriptor,
        IHaqqScoringService,
        ITransientService
    {
        private readonly IHaqqExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="HaqqExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="HaqqExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IHaqqExplorerClient"/>.</param>
        ...
        public HaqqExplorerService(
            IOptions<HaqqExplorerSettings> settings,
            IHaqqExplorerClient client,
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            // (await _defiLlamaService.GetTokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToIslm() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<HaqqExplorerAccountNormalTransactions, HaqqExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<HaqqExplorerAccountERC20TokenEvents, HaqqExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new HaqqStatCalculator(
                    request.Address,
