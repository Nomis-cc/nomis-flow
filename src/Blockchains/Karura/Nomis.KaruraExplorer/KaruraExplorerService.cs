// ------------------------------------------------------------------------------------------------------
// <copyright file="KaruraExplorerService.cs" company="Nomis">
...
using Nomis.KaruraExplorer.Calculators;
using Nomis.KaruraExplorer.Interfaces;
using Nomis.KaruraExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IKaruraScoringService"/>
    internal sealed class KaruraExplorerService :
        BlockchainDescriptor,
        IKaruraScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IKaruraExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="KaruraExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="KaruraExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IKaruraExplorerClient"/>.</param>
        ...
        public KaruraExplorerService(
            IOptions<KaruraExplorerSettings> settings,
            IKaruraExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "karura";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "karura";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToKar() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<KaruraExplorerAccountNormalTransactions, KaruraExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<KaruraExplorerAccountERC20TokenEvents, KaruraExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new KaruraStatCalculator(
                    request.Address,
