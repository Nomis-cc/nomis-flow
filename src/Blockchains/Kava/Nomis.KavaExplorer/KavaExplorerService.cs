// ------------------------------------------------------------------------------------------------------
// <copyright file="KavaExplorerService.cs" company="Nomis">
...
using Nomis.KavaExplorer.Calculators;
using Nomis.KavaExplorer.Interfaces;
using Nomis.KavaExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IKavaScoringService"/>
    internal sealed class KavaExplorerService :
        BlockchainDescriptor,
        IKavaScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IKavaExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="KavaExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="KavaExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IKavaExplorerClient"/>.</param>
        ...
        public KavaExplorerService(
            IOptions<KavaExplorerSettings> settings,
            IKavaExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "kava";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "kava";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToKava() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<KavaExplorerAccountNormalTransactions, KavaExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<KavaExplorerAccountERC20TokenEvents, KavaExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new KavaStatCalculator(
                    request.Address,
