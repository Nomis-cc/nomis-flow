// ------------------------------------------------------------------------------------------------------
// <copyright file="AndromedaExplorerService.cs" company="Nomis">
...
using Nomis.AndromedaExplorer.Calculators;
using Nomis.AndromedaExplorer.Interfaces;
using Nomis.AndromedaExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IMetisScoringService"/>
    internal sealed class AndromedaExplorerService :
        BlockchainDescriptor,
        IMetisScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IAndromedaExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="AndromedaExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="AndromedaExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IAndromedaExplorerClient"/>.</param>
        ...
        public AndromedaExplorerService(
            IOptions<AndromedaExplorerSettings> settings,
            IAndromedaExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "metis";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "metis-token";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToMetis() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<AndromedaExplorerAccountNormalTransactions, AndromedaExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<AndromedaExplorerAccountERC20TokenEvents, AndromedaExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new MetisStatCalculator(
                    request.Address,
