// ------------------------------------------------------------------------------------------------------
// <copyright file="MetisStatCalculator.cs" company="Nomis">
...
namespace Nomis.AndromedaExplorer.Calculators
{
    /// <summary>
    /// Metis wallet stats calculator.
    /// </summary>
    internal sealed class MetisStatCalculator :
        IStatCalculator<MetisWalletStats, MetisTransactionIntervalData>
    {
        private readonly string _address;
        private readonly decimal _balance;
        private readonly decimal _usdBalance;
        private readonly IEnumerable<AndromedaExplorerAccountNormalTransaction> _transactions;
        private readonly IEnumerable<AndromedaExplorerAccountERC20TokenEvent> _erc20TokenTransfers;
        ...
        public MetisStatCalculator(
            string address,
            decimal balance,
            decimal usdBalance,
            IEnumerable<AndromedaExplorerAccountNormalTransaction> transactions,
            IEnumerable<AndromedaExplorerAccountERC20TokenEvent> erc20TokenTransfers,
        ...
        public MetisWalletStats GetStats()
        {
            ...
            var turnoverIntervals = IStatCalculator<MetisTransactionIntervalData>
                .GetTurnoverIntervals(turnoverIntervalsDataList, _transactions.Min(x => x.TimeStamp!.ToDateTime())).ToList();

            return new()
            {
                NativeBalance = _balance.ToMetis(),
                ...
                WalletTurnover = _transactions.Sum(x => decimal.TryParse(x.Value, out decimal value) ? value.ToMetis() : 0),
                TurnoverIntervals = turnoverIntervals,
                BalanceChangeInLastMonth = IStatCalculator<MetisTransactionIntervalData>.GetBalanceChangeInLastMonth(turnoverIntervals),
                BalanceChangeInLastYear = IStatCalculator<MetisTransactionIntervalData>.GetBalanceChangeInLastYear(turnoverIntervals),
