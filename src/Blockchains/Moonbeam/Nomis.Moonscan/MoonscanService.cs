...
namespace Nomis.Moonscan
{
        ...
        ITransientService
    {
        ...
        /// </summary>
        /// <param name="settings"><see cref="MoonscanSettings"/>.</param>
        ...
        public MoonscanService(
            IOptions<MoonscanSettings> settings,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "moonbeam";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "moonbeam";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToGlmr() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<MoonscanAccountNormalTransactions, MoonscanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<MoonscanAccountInternalTransactions, MoonscanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<MoonscanAccountERC20TokenEvents, MoonscanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<MoonscanAccountERC721TokenEvents, MoonscanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
