// ------------------------------------------------------------------------------------------------------
// <copyright file="MoonriverStatCalculator.cs" company="Nomis">
...
namespace Nomis.MoonriverMoonscan.Calculators
{
    /// <summary>
    /// Moonriver wallet stats calculator.
    /// </summary>
    internal sealed class MoonriverStatCalculator :
        IStatCalculator<MoonriverWalletStats, MoonriverTransactionIntervalData>
    {
        private readonly string _address;
        private readonly decimal _balance;
        private readonly decimal _usdBalance;
        private readonly IEnumerable<MoonriverMoonscanAccountNormalTransaction> _transactions;
        private readonly IEnumerable<MoonriverMoonscanAccountInternalTransaction> _internalTransactions;
        private readonly IEnumerable<IMoonriverMoonscanAccountNftTokenEvent> _tokenTransfers;
        private readonly IEnumerable<MoonriverMoonscanAccountERC20TokenEvent> _erc20TokenTransfers;
        ...
        public MoonriverStatCalculator(
            string address,
            decimal balance,
            decimal usdBalance,
            IEnumerable<MoonriverMoonscanAccountNormalTransaction> transactions,
            IEnumerable<MoonriverMoonscanAccountInternalTransaction> internalTransactions,
            IEnumerable<IMoonriverMoonscanAccountNftTokenEvent> tokenTransfers,
            IEnumerable<MoonriverMoonscanAccountERC20TokenEvent> erc20TokenTransfers,
        ...
        public MoonriverWalletStats GetStats()
        {
            ...
            var turnoverIntervals = IStatCalculator<MoonriverTransactionIntervalData>
                .GetTurnoverIntervals(turnoverIntervalsDataList, _transactions.Min(x => x.TimeStamp!.ToDateTime())).ToList();

            return new()
            {
                NativeBalance = _balance.ToMovr(),
                ...
                WalletTurnover = _transactions.Sum(x => decimal.TryParse(x.Value, out decimal value) ? value.ToMovr() : 0),
                TurnoverIntervals = turnoverIntervals,
                BalanceChangeInLastMonth = IStatCalculator<MoonriverTransactionIntervalData>.GetBalanceChangeInLastMonth(turnoverIntervals),
                BalanceChangeInLastYear = IStatCalculator<MoonriverTransactionIntervalData>.GetBalanceChangeInLastYear(turnoverIntervals),
                ...
                NftTrading = (soldSum - buySum).ToMovr(),
                NftWorth = nftWorth.ToMovr(),
