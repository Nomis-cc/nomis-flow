// ------------------------------------------------------------------------------------------------------
// <copyright file="OasisExplorerService.cs" company="Nomis">
...
using Nomis.OasisExplorer.Calculators;
using Nomis.OasisExplorer.Interfaces;
using Nomis.OasisExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IOasisScoringService"/>
    internal sealed class OasisExplorerService :
        BlockchainDescriptor,
        IOasisScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IOasisExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="OasisExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="OasisExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IOasisExplorerClient"/>.</param>
        ...
        public OasisExplorerService(
            IOptions<OasisExplorerSettings> settings,
            IOasisExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "oasis";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "oasis-network";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToRose() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<OasisExplorerAccountNormalTransactions, OasisExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<OasisExplorerAccountERC20TokenEvents, OasisExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new OasisStatCalculator(
                    request.Address,
