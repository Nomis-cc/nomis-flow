// ------------------------------------------------------------------------------------------------------
// <copyright file="PoaExplorerService.cs" company="Nomis">
...
using Nomis.PoaExplorer.Calculators;
using Nomis.PoaExplorer.Interfaces;
using Nomis.PoaExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IPoaScoringService"/>
    internal sealed class PoaExplorerService :
        BlockchainDescriptor,
        IPoaScoringService,
        ITransientService
    {
        private readonly IPoaExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="PoaExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="PoaExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IPoaExplorerClient"/>.</param>
        ...
        public PoaExplorerService(
            IOptions<PoaExplorerSettings> settings,
            IPoaExplorerClient client,
        ...
        /// </summary>
        public string CoingeckoNativeTokenId => "poa-network";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToPoa() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<PoaExplorerAccountNormalTransactions, PoaExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<PoaExplorerAccountERC20TokenEvents, PoaExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new PoaStatCalculator(
                    request.Address,
