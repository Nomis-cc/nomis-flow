// ------------------------------------------------------------------------------------------------------
// <copyright file="RskExplorerService.cs" company="Nomis">
...
using Nomis.RskExplorer.Calculators;
using Nomis.RskExplorer.Interfaces;
using Nomis.RskExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IRskScoringService"/>
    internal sealed class RskExplorerService :
        BlockchainDescriptor,
        IRskScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IRskExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="RskExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="RskExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IRskExplorerClient"/>.</param>
        ...
        public RskExplorerService(
            IOptions<RskExplorerSettings> settings,
            IRskExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "rsk";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "rootstock";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToRbtc() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<RskExplorerAccountNormalTransactions, RskExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<RskExplorerAccountERC20TokenEvents, RskExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new RskStatCalculator(
                    request.Address,
