// ------------------------------------------------------------------------------------------------------
// <copyright file="ShidenExplorerService.cs" company="Nomis">
...
using Nomis.ShidenExplorer.Calculators;
using Nomis.ShidenExplorer.Interfaces;
using Nomis.ShidenExplorer.Interfaces.Extensions;
...
{
    /// <inheritdoc cref="IShidenScoringService"/>
    internal sealed class ShidenExplorerService :
        BlockchainDescriptor,
        IShidenScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IShidenExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="ShidenExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="ShidenExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IShidenExplorerClient"/>.</param>
        ...
        public ShidenExplorerService(
            IOptions<ShidenExplorerSettings> settings,
            IShidenExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "shiden";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "shiden";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToSdn() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<ShidenExplorerAccountNormalTransactions, ShidenExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<ShidenExplorerAccountERC20TokenEvents, ShidenExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new ShidenStatCalculator(
                    request.Address,
