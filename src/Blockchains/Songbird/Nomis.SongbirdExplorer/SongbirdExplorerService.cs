// ------------------------------------------------------------------------------------------------------
// <copyright file="SongbirdExplorerService.cs" company="Nomis">
...
using Nomis.SongbirdExplorer.Calculators;
...
{
    /// <inheritdoc cref="ISongbirdScoringService"/>
    internal sealed class SongbirdExplorerService :
        BlockchainDescriptor,
        ISongbirdScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly ISongbirdExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="SongbirdExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="SongbirdExplorerSettings"/>.</param>
        /// <param name="client"><see cref="ISongbirdExplorerClient"/>.</param>
        ...
        public SongbirdExplorerService(
            IOptions<SongbirdExplorerSettings> settings,
            ISongbirdExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "songbird";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "songbird";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToSgb() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<SongbirdExplorerAccountNormalTransactions, SongbirdExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<SongbirdExplorerAccountERC20TokenEvents, SongbirdExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new SongbirdStatCalculator(
                    request.Address,
