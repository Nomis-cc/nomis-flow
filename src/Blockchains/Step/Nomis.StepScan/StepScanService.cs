// ------------------------------------------------------------------------------------------------------
// <copyright file="StepScanService.cs" company="Nomis">
...
using Nomis.StepScan.Calculators;
...
{
    /// <inheritdoc cref="IStepScoringService"/>
    internal sealed class StepScanService :
        BlockchainDescriptor,
        IStepScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IStepScanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="StepScanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="StepScanSettings"/>.</param>
        /// <param name="client"><see cref="IStepScanClient"/>.</param>
        ...
        public StepScanService(
            IOptions<StepScanSettings> settings,
            IStepScanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "step";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "step-app-fitfi";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToFitfi() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<StepScanAccountNormalTransactions, StepScanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<StepScanAccountERC20TokenEvents, StepScanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new StepStatCalculator(
                    request.Address,
