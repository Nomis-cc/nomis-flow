// ------------------------------------------------------------------------------------------------------
// <copyright file="VelasExplorerService.cs" company="Nomis">
...
using Nomis.VelasExplorer.Calculators;
using Nomis.VelasExplorer.Interfaces;
using Nomis.VelasExplorer.Interfaces.Extensions;
using Nomis.VelasExplorer.Interfaces.Models;
using Nomis.VelasExplorer.Settings;

namespace Nomis.VelasExplorer
{
    /// <inheritdoc cref="IVelasScoringService"/>
    internal sealed class VelasExplorerService :
        BlockchainDescriptor,
        IVelasScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IVelasExplorerClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="VelasExplorerService"/>.
        /// </summary>
        /// <param name="settings"><see cref="VelasExplorerSettings"/>.</param>
        /// <param name="client"><see cref="IVelasExplorerClient"/>.</param>
        ...
        public VelasExplorerService(
            IOptions<VelasExplorerSettings> settings,
            IVelasExplorerClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "velas";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "velas";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance =
                (await _defiLlamaService.TokensPriceAsync(new List<string?> { $"coingecko:{CoingeckoNativeTokenId}" }).ConfigureAwait(false))?.TokensPrices[$"coingecko:{CoingeckoNativeTokenId}"].Price * balanceWei?.ToVlx() ?? 0;
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var transactions = (await _client.GetTransactionsAsync<VelasExplorerAccountNormalTransactions, VelasExplorerAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            await Task.Delay(100, cancellationToken).ConfigureAwait(false);
            var erc20Tokens = (await _client.GetTransactionsAsync<VelasExplorerAccountERC20TokenEvents, VelasExplorerAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new VelasStatCalculator(
                    request.Address,
